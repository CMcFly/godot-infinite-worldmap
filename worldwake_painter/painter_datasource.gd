...
func paint_line(from_cell: Vector2i, to_cell: Vector2i, terrain_id: int, brush_radius: int) -> void:
	...
	var radius: int = maxi(0, brush_radius)
	...
	if steps <= 0:
		...
		for i in range(steps + 1):
			...
			var x: int = roundi(lerpf(float(from_cell.x), float(to_cell.x), t))
			var y: int = roundi(lerpf(float(from_cell.y), float(to_cell.y), t))
	...
	for oy in range(-radius, radius + 1):
		for ox in range(-radius, radius + 1):
			...
			if x < 0 or y < 0 or x >= WORLD_SIZE.x or y >= WORLD_SIZE.y:
				continue
			var cell_key: int = y * WORLD_SIZE.x + x
			if terrain_id < 0:
...
func _base_terrain(world_x: float, world_y: float) -> int:
	...
	var nx: float = (world_x - WORLD_SIZE.x * 0.5) / (WORLD_SIZE.x * 0.5)
	var ny: float = (world_y - WORLD_SIZE.y * 0.5) / (WORLD_SIZE.y * 0.5)
	var edge: float = maxf(absf(nx), absf(ny))
	var edge_penalty: float = 0.0
	if edge > 0.67:
		edge_penalty = pow((edge - 0.67) / 0.33, 2.0) * 1.35

	var continent: float = _continent_noise.get_noise_2d(world_x, world_y)
	var detail: float = _detail_noise.get_noise_2d(world_x, world_y)
	var land: float = continent * 0.82 + detail * 0.22 - edge_penalty
...
func get_biome_image(camera_zoomed_size: Vector2i):
	var width: int = maxi(1, camera_zoomed_size.x)
	var height: int = maxi(1, camera_zoomed_size.y)
	var buffer := PackedByteArray()
	buffer.resize(width * height * 3)

	_edit_mutex.lock()
	var edit_snapshot: Dictionary = _edits.duplicate()
	_edit_mutex.unlock()

	var local_zoom: float = maxf(zoom, 0.00001)
	var write_index: int = 0
	for py in range(height):
		var world_y: float = (offset.y + float(py)) / local_zoom
		var cell_y: int = floori(world_y)
		for px in range(width):
			var world_x: float = (offset.x + float(px)) / local_zoom
			var cell_x: int = floori(world_x)
			var terrain_id: int = -1

			if cell_x >= 0 and cell_y >= 0 and cell_x < WORLD_SIZE.x and cell_y < WORLD_SIZE.y:
				var cell_key: int = cell_y * WORLD_SIZE.x + cell_x
				terrain_id = int(edit_snapshot.get(cell_key, NO_EDIT))
				if terrain_id == NO_EDIT:
					terrain_id = _base_terrain(world_x, world_y)

			var color_bytes: Array = OUTSIDE_COLOR if terrain_id < 0 else TERRAIN_COLORS[terrain_id]
