...
# GDScript-only session factory used by the WorldWake Painter prototype.
# The upstream fork also preloaded its C# datasource here, which makes a
# standard (non-.NET) Godot project unnecessarily depend on C# support.
